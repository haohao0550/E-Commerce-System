...
export const RegisterForm = () => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (phone && !/^[0-9]{10,11}$/.test(phone)) {
      setClientError('Phone must contain 10 to 11 digits.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <p className="mb-10 text-on-surface-variant">
        Create an account for early access to drops and exclusive releases.
      </p>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <StatusMessage message={clientError || error} tone="error" />
      ...
      <p className="mt-10 text-center text-sm font-medium text-on-surface-variant">
        ...
        <Link href={ROUTES.login} className="font-bold text-primary underline underline-offset-4 transition-colors hover:text-on-surface">
          Sign in
        </Link>
      </p>
    </motion.div>
