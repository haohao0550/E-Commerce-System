...
export class CartRepo implements ICartRepo {
	...
	async findByUserId(userId: string, skip?: number, take?: number): Promise<CartItemWithVariant[]> {
	...
	async aggregateByUserId(userId: string): Promise<{ itemsCount: number; totalQuantity: number }> {
	...
	async createMany(data: Prisma.CartCreateManyInput[], tx?: Prisma.TransactionClient): Promise<void> {
	...
	async update(id: string, data: Prisma.CartUpdateInput, tx?: Prisma.TransactionClient): Promise<Cart> {
	...
	async deleteByVariantId(userId: string, variantId: string, tx?: Prisma.TransactionClient): Promise<void> {
		await this.getClient(tx).cart.deleteMany({
