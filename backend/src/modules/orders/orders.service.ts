...
import type { IOrdersRepo, OrderItemCreateRepoInput, CreateOrderTransactionInput } from './orders.repo.interface.js'
  ...
  async createOrder(userId: string, data: CreateOrderDTO) {
    ...
    if (data.couponId) {
      ...
      if (coupon.minOrderValue && totalPrice < Number(coupon.minOrderValue)) {
        throw new BadRequestError(`Order has not reached the minimum value (${coupon.minOrderValue}) to apply this coupon code`)
      }
